(function() {
    ...
    1: [function(require, module, exports) {
        ...
        var STORAGE_KEY = 'TrustboxSplitTest';
        ...
        function readStorageOrCookie() {
            var storedValue = tryReadLocalStorage();
            if (storedValue) {
                return storedValue;
            }
            var name = STORAGE_KEY + '=';
            var wholeCookieString = document.cookie;
            var storageStart = wholeCookieString.indexOf(name);
            if (storageStart > -1) {
                return wholeCookieString.substring(storageStart + name.length).split(';')[0];
            }
        }

        function tryWriteLocalStorage(value) {
            try {
        ...
        function setStorageOrCookie(settings, expiry) {
            if (!tryWriteLocalStorage(settings)) {
                document.cookie = [STORAGE_KEY + '=' + settings, 'path=/', 'domain=' + getTopLevelDomain(), 'expires=' + expiry, 'samesite=strict'].join('; ');
            }
        }

        function getStoredSettings(businessUnitId) {
            var serializedData = void 0;
            try {
                serializedData = readStorageOrCookie();
                var settings = JSON.parse(decodeURIComponent(serializedData));
                var expiresAt = new Date(settings.expiry);
                if (settings.businessUnitId === businessUnitId && expiresAt > new Date()) {
                    return settings;
                }
            } catch (e) {
                // Botched storage, remove
                if (serializedData) {
                    try {
                        localStorage.removeItem(STORAGE_KEY);
                        var domain = 'domain=' + window.location.hostname.replace(/^.*\.([^.]+\.[^.]+)/, '$1');
                        document.cookie = STORAGE_KEY + '=;expires=Thu, 01 Jan 1970 00:00:01 GMT;path=/;' + domain;
                    } catch (e) {
                        // Nothing to do
                    }
                }
        ...
        // Returns an 'expires=...' string to pass along to the widget iframe for creation of cookies on widget.tp.com
        function saveSettings(settings) {
            if (settings) {
                ...
                setStorageOrCookie(encodeURIComponent(JSON.stringify(settings)), settings.expiry);
                return 'expires=' + settings.expiry;
            }
        }
        ...
        function getABSettingsAsync(businessUnitId, callback) {
            var storedSettings = getStoredSettings(businessUnitId);
            var sessionDuration = 1800000; // 30 mins in millisecs
            if (storedSettings && (!storedSettings.sessionStart || +new Date() < storedSettings.sessionStart + sessionDuration)) {
                var expires = 'expires=' + storedSettings.expiry;
                callback(storedSettings, expires);
                return;
            }
            ...
            if (settingsCallbackQueue.length > 1) {
                ...
                xhr.xhrGet({
                    url: '#{WidgetRoot}/v1/ratios/' + businessUnitId,
                    success: function success(data) {
                        var settingsFromApi = generateSettingsFromApiData(data, businessUnitId);
                        if (storedSettings && storedSettings.testId === settingsFromApi.testId) {
                            // New session with same settings
                            storedSettings.session = generateGuid();
                            storedSettings.sessionStart = +new Date();
                            return processCallbackQueue(storedSettings);
                        } else {
        ...
        module.exports = {
            getABSettingsAsync: getABSettingsAsync
        };
    ...
    3: [function(require, module, exports) {
        ...
        function getProductSnippet(_ref2) {
            ...
            var url = '#{WidgetRoot}' + path + '?sku=' + encodeURIComponent(sku) + '&numberOfReviews=' + (reviewnumber || 10) + (name ? '&productName=' + name : '') + '&language=' + language + '&templateId=' + templateId + '&url=' + encodeURIComponent(window.location.origin + window.location.pathname);
    ...
    4: [function(require, module, exports) {
        ...
        var safeToLower = function safeToLower(s) {
            ...
            _createClass(WidgetManagement, [{
                ...
                value: function checkWidgetDisplayAndUpdateDataset(element) {
                    ...
                    if (!this.abSettings || !('group' in dataset)) {
                        return true;
                    } else if (safeToLower(this.abSettings.group) === safeToLower(dataset.group)) {
                        return true;
                    } else if (safeToLower(this.abSettings.group) === _constants.OFF) {
